...
class Manage
{
	...
	public function manageRecordWithPagination($table,$pno){
		...
		if ($table == "doctor") {
			$sql = "SELECT p.did,p.Address,p.Doctor_name as doctor, c.Doctor_name as parent, p.status FROM doctor p LEFT JOIN doctor c ON p.parent_cat=c.did ".$a["limit"];
		}else{
	...
	public function deleteRecord($table,$pk,$id){
		if($table == "doctor"){
			$pre_stmt = $this->con->prepare("SELECT ".$id." FROM doctor WHERE parent_cat = ?");
			$pre_stmt->bind_param("i",$id);
			$pre_stmt->execute();
			$result = $pre_stmt->get_result() or die($this->con->error);
			if ($result->num_rows > 0) {
				return "DEPENDENT_Doctor";
			}else{
		...
		if ($invoice_no != null) {
			for ($i=0; $i < count($ar_price) ; $i++) {
				...
				if ($rem_qty < 0) {
					...
					$sql = "UPDATE medicine SET product_stock = '$rem_qty' WHERE medicine_name = '".$ar_pro_name[$i]."'";
					$this->con->query($sql);
				}


				$insert_medicine = $this->con->prepare("INSERT INTO `prescription`( `medicine_name`, `price`, `qty`)
				 VALUES (?,?,?)");
				$insert_medicine->bind_param("sdd",$ar_pro_name[$i],$ar_price[$i],$ar_qty[$i]);
				$insert_medicine->execute() or die($this->con->error);
			}
...
//$obj = new Manage();
//echo "<pre>";
//print_r($obj->manageRecordWithPagination("doctor",1));
//echo $obj->deleteRecord("doctor","did",14);
//print_r($obj->getSingleRecord("doctor","did",1));
//echo $obj->update_record("doctor",["did"=>1],["parent_cat"=>0,"Doctor_name"=>"Electro","status"=>1]);
?>